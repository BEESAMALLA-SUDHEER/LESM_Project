...
//5
	...
	public void transferEmployeeToSameDesignation(String lancesoftId, String tansferLancesoftId, String Location,
			Double Salary, Principal principal) {
		...
		History historyEmp = new History(trasferEmp.getLancesoft(), trasferEmp.getFirstName(), trasferEmp.getLastName(),
				...
				trasferEmp.getSupervisor(), trasferEmp.getEmployeeType(), UpdatedStatus.TRANSFER, LocalDate.now(),
				updatedBy);
		...
		for (Salary sall : salrec) {
			...
			salaryRepository
					.save(new Salary(Salary1, sall.getCreatedAt(), LocalDate.now(), sall.getCreatedBy(), trasferEmp));
			break;
		}
	}}
...
//	}
	...
	public List<MasterEmployeeDetails> consultantsinManagerDashboard(Principal principal) {
		...
		for (MasterEmployeeDetails leads_id : leads) {
			 ...
			 consultants = this.employeeDetailsRepository
