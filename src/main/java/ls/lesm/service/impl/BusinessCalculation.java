		...
		try {
			...
			if (fromDate == null && toDate == null) {
				...
				if (fromDate.getDayOfMonth() != 1) {
					...
					if (fromDate.getMonthValue() < 12) {

						fromDate = LocalDate.of(fromDate.getYear(), fromDate.getMonthValue() + 1, 1);
					}

					else {
						fromDate = LocalDate.of(fromDate.getYear() + 1, 1, 1);
			...
			if (day != 1) {
				...
				if (toDate.getMonthValue() == 12) {

					fromDate = LocalDate.of(year + 1, 1, 1);
				} else {

					fromDate = LocalDate.of(year, month + 1, 1);
			...
			// ..................................................................
			// fromDate = LocalDate.of(year, month, day);(previous)
			...
			if (Date1.isBefore(toDate) && Date2.isAfter(fromDate)) {
				...
				if (Date1.isBefore(fromDate)) {
					...
					if (Date1.getDayOfMonth() != 1) {
						...
						remaining_days = 30 - Date1.getDayOfMonth();
...
//					cl.setTotalEarningAtclient(Bill_at_client);
	...
	//				clientDetail.save(cl);
					...
					cl.setTotalEarningAtclient((cl.getClientSalary() * client_tenure)
							+ perday * v3 + remaining_days * perday);
