...
public class CalculationController {
...

	{
		...
		// MasterEmployeeDetails emp2 =
		// this.masterEmployeeDetailsRepository.findByLancesoft(emp1);

		// int id = emp2.getEmpId();

		Double profit_or_loss = businessCalculation.Employee_cal(id, dateRange.getFromDate(), dateRange.getToDate());
...
//	}
	...
	public ResponseEntity<Double>  generalManagerCalculationManagerCalculation(Principal principal, @RequestBody DateRange dateRange) {
		try {
		String loggedIn = principal.getName();
		MasterEmployeeDetails employee = this.masterEmployeeDetailsRepository.findByLancesoft(loggedIn);
		int id = employee.getEmpId();
